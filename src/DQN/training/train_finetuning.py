...
for episode in tqdm(range(CONFIG['episodes']), desc="Fine-Tuning Training"):
    ...
    # Sichere TD-Error-Normalisierung auf Episodenbasis
    if len(td_error_history) < 5 or not np.isfinite(np.mean(td_error_history[-5:])):
        td_error_norm_ep = 0.0
    else:
        td_error_norm_ep = float(np.clip(avg_td_error / (np.mean(td_error_history[-50:]) + 1e-6), 0, 10))
