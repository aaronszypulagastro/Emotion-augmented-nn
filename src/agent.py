...
class PlasticLinear (nn.Module):
    ...
    @torch.no_grad()
    def plasticity_step(self, mod: float, eta: float = 1e-3, decay: float = 0.997, clip: float = 0.1):
        """ 
        ...
        post = torch.tanh(self._last_post)      # [B, out]
...
class DQNAgent:    # Agent-Parameter
    def __init__(self, state_dim, action_dim, config=None, emotion_engine=None):
        ...
        self.emotion_enabled = getattr(self.cfg, 'emotion_enabled', False)

        if self.emotion_enabled:
            try: 
                self.emotion = EmotionEngine(self.cfg)
                self.emotion_gain = 0.4 
                print("Emotion Engine aktiviert.")

                # Debug: Überprüfen, ob Engine wirklich aktiv ist 
                if self.emotion is not None: 
                    print(f'[Debug] EmotionEngine Typ: {type(self.emotion)}')

                else: 
                    print("[Debug] EmotionEngine NICHT initialisiert.")
        ...
        def _emotion_gain(self) -> float:
            ...
            shift = (e - 0.05) * 2.0
            # Basiseinfluss
            gain = 1.0 + self.emotion_gain * shift
            return float(np.clip(gain, 0.8, 1.2))
